...
`HTTP/1.0`加入了请求头，更多请求方法，响应内容不再局限于超文本。

`HTTP/1.1`包含了太多细节和可选的部分，导致其过于庞大。没有浏览器实现了所有细节，实际上很多细节都无法弄清。

`rfc2616`太老，`rfc7230`-`rfc7235`对`HTTP/1`做了大量修订。

`HTTP/2`起源于`SPDY`，后者是谷歌牵头开发的协议。`HTTP/2`是二进制协议，删除或减少可选的部分，并且将不再使用小版本号。
...
浏览器再次请求时，首部带上 `If-None-Matched`（存储的是Etag值），后端再次生成etag值并与该值比对，如果两个值相同，生成的内容不反悔，只返回 `Status: 304 Not Modified`，客户端使用本地缓存。
